async function loadGroups() {
    ...
    if (memberError) return console.warn("Error loading group memberships:", memberError);

    if (!members || members.length === 0) {
        document.getElementById('html-groups-list').innerHTML = `<p class="text-slate-500 text-xs uppercase font-bold pt-4">${t('no_groups')}</p>`;
        return;
    }

    const groupIds = [...new Set(members.map(m => m.group_id))];

    // 2. Obtener los detalles de esos grupos de la tabla pública
    const { data: groupData, error: groupError } = await _supabase
        .from('groups')
        .select('id, name, code')
        .in('id', groupIds);

    if (groupError) return console.warn("Error loading group details:", groupError);

    const uniqueGroups = groupData || [];

    document.getElementById('html-groups-list').innerHTML = uniqueGroups.map(g => `
        <div onclick="showGroupDetail('${g.id}', '${g.name}')" class="p-6 card-dark rounded-3xl cursor-pointer border-l-8 border-indigo-600 shadow-md">
...
async function updateProfileName() {
    ...
    if (!name) return;
    await _supabase.schema('flexible_carpooling').from('flexible_members').update({ display_name: name }).eq('user_id', user.id);
...
async function loadUserData() {
    const { data } = await _supabase.schema('flexible_carpooling').from('flexible_members').select('aporta_coche, display_name').eq('user_id', user.id).limit(1).single();
    if (data) {
...
async function createGroup() {
    ...
    const { data: g } = await _supabase.from('groups').insert([{ name, code: code, owner_id: user.id }]).select().single();
    await _supabase.schema('flexible_carpooling').from('flexible_members').insert([{ group_id: g.id, user_id: user.id, user_email: user.email, aporta_coche: true }]);
...
async function joinGroup() {
    ...
    try {
        const { data: g } = await _supabase.from('groups').select('id').eq('code', code).maybeSingle();
        if (!g) {
        ...
        const { data: memberCheck } = await _supabase.schema('flexible_carpooling').from('flexible_members')
            .select('id')
            .eq('group_id', g.id)
            .eq('user_id', user.id);
        ...
        const { error } = await _supabase.schema('flexible_carpooling').from('flexible_members').insert([
